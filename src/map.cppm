...
auto map(Awaitable&& awaitable, Func&& func) {
    return map_awaitable<std::remove_cvref_t<Awaitable>, std::remove_cvref_t<Func>>(
        std::move(awaitable), std::move(func)
    );
}

template<stream_awaitable StreamAwaitable, typename Func>
auto map(StreamAwaitable&& stream, Func&& func) {
    return map_stream_awaitable<std::remove_cvref_t<StreamAwaitable>, std::remove_cvref_t<Func>>(
        std::move(stream), std::move(func)
    );
}

template<typename Func>
auto map(Func&& func) {
    return map_composable<std::remove_cvref_t<Func>>(std::move(func));
}

template<awaitable Awaitable, typename Func>
auto operator|(Awaitable&& awaitable, map_composable<Func>&& mapper) {
    return map_awaitable<std::remove_cvref_t<Awaitable>, Func>(
        std::move(awaitable), std::move(mapper.func)
    );
}

template<stream_awaitable StreamAwaitable, typename Func>
auto operator|(StreamAwaitable&& stream, map_composable<Func>&& mapper) {
    return map_stream_awaitable<std::remove_cvref_t<StreamAwaitable>, Func>(
        std::move(stream), std::move(mapper.func)
    );
}
