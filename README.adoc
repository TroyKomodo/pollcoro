...
* link:examples/interop_asio.cc[interop_asio.cc] — Bidirectional interop with ASIO using `from_asio` and `to_asio`
...
pollcoro provides full bidirectional interoperability with ASIO through `<pollcoro/asio.hpp>`.

==== `pollcoro::from_asio` — Use ASIO inside pollcoro

Convert an ASIO awaitable factory into a pollcoro awaitable:

[source,cpp]
----
#include <pollcoro/asio.hpp>

pollcoro::task<int> use_asio_timer(asio::any_io_executor executor) {
    ...
    // Get a value from ASIO
    int result = co_await pollcoro::from_asio<int>(executor, [&]() -> asio::awaitable<int> {
        co_return 42;
    });
...
NOTE: You must run `io_context::run()` in a separate thread to drive the ASIO operations while `block_on` drives the pollcoro task.

==== `pollcoro::to_asio` — Use pollcoro inside ASIO
...
asio::awaitable<int> use_pollcoro_task() {
    ...
    int result = co_await pollcoro::to_asio(
        executor, pollcoro_task(), asio::use_awaitable
    );
...
==== `pollcoro::asio_scheduler` — Use pollcoro with `to_resumable`

A scheduler adapter for using `to_resumable` with ASIO-based event loops:

[source,cpp]
----
pollcoro::asio_scheduler sched{ctx.get_executor()};
