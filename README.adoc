...
You can create your own waker types by implementing the `wake` and `clone` methods.
...
class my_waker_t {
    ...
    // Optional method to clone the waker
    // If not provided the waker is assumed to be trivially copyable
    waker clone() {
...
auto waker = pollcoro::waker(my_waker);
auto some_task = /* ... */;

auto result = some_task.on_poll(waker);
