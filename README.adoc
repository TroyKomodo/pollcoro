...
=== `pollcoro::stream_from`

Lightweight helpers to create stream-like awaitables from iterators or containers. Useful for yield-from with existing collections.
...
co_yield pollcoro::stream_from(vec.begin(), vec.end());

// From range/container
co_yield pollcoro::stream_from(std::array{4, 5, 6});
co_yield pollcoro::stream_from(vec);
----
