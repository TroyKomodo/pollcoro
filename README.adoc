...
struct poll_awaitable {
    pollcoro::pollable_state<int> on_poll(pollcoro::waker& w) {
        ...
        waker_ = w.clone();  // Store waker to signal "poll me again"
...
struct my_timer {
    ...
    void register_callback(time_point deadline, std::function<void()> callback) {
        // Maybe use a thread pool, or some other efficent way to schedule the callback
...
co_await pollcoro::sleep_until<my_timer<>>(std::chrono::steady_clock::now() + std::chrono::seconds(5));
----
...
public:
    pollcoro::pollable_state<void> on_poll(const pollcoro::waker& w) {
        if (is_ready()) {
...
auto result = some_task.on_poll(pollcoro::waker(my_waker));
// do something with the result

----

==== Note about lifetimes and ownership

The `waker` class does not have a destructor or any clone methods. This is because wakers need to be valid as long as the `awaitable` is valid. When the `awaitable` is destroyed the waker can be destroyed as well.

The single_event task is a good example of this, there is a shared_ptr between the awaitable and the setter. 

If the awaitable (receiver) is destroyed, the waker inside the `setter` will be unset so that if another thread calls `setter.set()`, the waker is not called.

Its up to the awaitable to ensure that the waker is not called after it has been destroyed.
...
public:
    ...
    pollcoro::pollable_state<void> on_poll(pollcoro::waker& w) {
        ...
        waker_->store(w, std::memory_order_release);
        if (!started_) {
            started_ = true;
