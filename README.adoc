...
NOTE: Requires CMake 3.28+ and a C++20 modules-capable compiler (Clang 16+, MSVC 19.34+). GCC has poor modules support and is not recommended.
...
* link:examples/mutex.cc[mutex.cc] — Async mutex and shared_mutex safe to hold across yield points
...
* link:examples/interop_cppcoro.cc[interop_cppcoro.cc] — Bidirectional interop with cppcoro using `to_pollable` and `to_resumable`
* link:examples/interop_asio.cc[interop_asio.cc] — Bidirectional interop with ASIO (adapters defined in example)
* link:examples/cxx_cppcoro_interop.cc[cxx_cppcoro_interop.cc] — Low-level manual interop with cppcoro via C API
* link:examples/reference.cc[reference.cc] — Using `ref` to poll a task without consuming it
...
NOTE: `mutex` and `shared_mutex` are **non-copyable and non-movable**. They must be declared in a stable location (class member, global, etc.) and accessed by reference. This ensures the internal state address remains valid for all guards.
...
See link:examples/interop_cppcoro.cc[interop_cppcoro.cc] for a complete working example with cppcoro.

=== ASIO Integration

pollcoro can interoperate bidirectionally with ASIO. The link:examples/interop_asio.cc[interop_asio.cc] example demonstrates how to build adapters for:
...
See link:examples/interop_asio.cc[interop_asio.cc] for complete ASIO integration examples including:
...
NOTE: ASIO adapters are not part of the pollcoro module itself. The `interop_asio.cc` example provides ready-to-use adapter implementations that you can copy into your project.
...
WARNING: **GCC has very poor C++20 modules support** and may or may not compile pollcoro. We recommend using Clang for the best experience.
